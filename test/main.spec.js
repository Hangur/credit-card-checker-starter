...
describe('findInvalidCards', () => {
  it('finds invalid cards', () => {
    const batchOfCards = [testInputs.valid[0], testInputs.valid[1], testInputs.invalid[5]]
    const expected = [testInputs.invalid[5]]
  ...
  it('returns no invalid cards when there are no invalid', () => {
    const batchOfCards = [testInputs.valid[2], testInputs.valid[3]]
    const expected = []
    const result = Validator.findInvalidCards(batchOfCards)
...
describe('idInvalidCardCompanies', () => {
  ...
  it('throws an error on uknown card company', () => {
    const batchOfInvalidCards = [testInputs.invalid[5], testInputs.invalid[6]]

    expect(() => {
      Validator.idInvalidCardCompanies(batchOfInvalidCards)
    }).toThrow(Error, 'Company not found!')
  })
})
